...
const routines = [["eatingPreferences","Food preferences or feeding support"],["sleepPreferences","Sleep routine and support"],["toiletingNotes","Toileting support"],["schoolEhcpNotes","School, work, day services or support-plan information"]];
...
export default function ChildSetupWizard({ api, familyId, allowed, initialProfile, serializeMedications, onComplete, first = false }) {
  ...
  const validate = () => {
    if (!basics.firstName.trim()) return "Please add the person's name.";
  ...
  const save = async () => {
    ...
    } catch (e) { setError(`${created.current.child ? "Profile created, but the care details have not saved. Keep this screen open and retry. " : ""}${e.message || "Please try again."}`); }
    finally { saving.current = false; setBusy(false); }
  };
  if (!open) return <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><h4 className="font-bold">Add another care profile</h4><p className="my-2 text-sm text-slate-600">Set up their profile, medicines and daily routines step by step.</p><button type="button" className={button} disabled={!allowed} onClick={()=>setOpen(true)}>Start profile setup</button>{!allowed && <p className="mt-2 text-sm">Your account cannot add another care profile right now.</p>}</section>;
  return <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
    <p className="text-xs font-bold uppercase tracking-wide text-teal-700">Care profile setup · Step {step+1} of {steps.length}</p>
